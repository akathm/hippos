...
with st.expander('About the Results'):
    ...
    st.warning('If you see a project is in *\"pending\"* status, this means that we are waiting on action from that team. Please direct them to check their emails: one or more responsible parties have been emailed with a link to complete further steps in the KYC process. If you see an individual is in *\"retry\"*, please ask them to attempt the KYC again at kyc.optimism.io/, they did not submit all required documents.')
